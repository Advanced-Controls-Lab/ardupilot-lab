...
noise[0:2] = 0
noise[2] = 0.8
lambda_array = (lamb**2) * np.identity(6)
u,s,v = scipy.linalg.svd(coeff_array.T, full_matrices=False)

print(u.shape, s.shape, v.shape)
s = np.diag(s)
sol_array = (v.dot(np.linalg.inv(s.dot(s) + lambda_array))).dot(s.dot(u.T))
outputs = sol_array.T.dot(noise)
outputs = np.linalg.pinv(coeff_array).dot(noise)
for i in range(0,outputs.shape[1]): 
    if noise[0] <0.01 or noise [1] < 0.01: 
