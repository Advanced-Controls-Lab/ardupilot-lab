...
points = 150
noise = np.random.normal(0.01,0.002,6*points).reshape((6,points))
noise[2][:] = 0.5
noise[3][:] = 0
noise[4][:] = 0
noise
...
for j in range(0, outputs.shape[1]):
    ...
    for i in range(0,outputs.shape[0],3):
        ...
        if previous_alpha[counter] == 0 or previous_beta[counter] == 0 or lowpass_filter == 0:
            ...
            alpha_angles2 = (outputs[i,j]/outputs[i+2,j]) 
            alpha_angles[i//3,j] = 0.1 * (np.remainder(alpha_angles2, np.pi) * (180/np.pi) - previous_alpha[counter]) +(previous_alpha[counter])
            beta_angles2 = (outputs[i+1,j]/outputs[i+2,j]) 
            beta_angles[i//3,j] = 0.1 *(np.remainder(beta_angles2, np.pi) * (180/np.pi) - previous_beta[counter]) + (previous_beta[counter])
            previous_alpha[counter] = alpha_angles[i//3,j]
            previous_beta[counter] = beta_angles[i//3,j] 
            counter+=1   
